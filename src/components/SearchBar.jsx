...
export default function SearchBar() {
  ...
  const setPharmacyOnly = useFilterStore((state) => state.setPharmacyOnly);
    ...
    <div className="bg-white shadow-sm">
      <div className="max-w-[1920px] mx-auto px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 py-4 md:pt-4 md:pb-6">
        ...
        <div className="hidden md:flex gap-4 items-center">
          {/* All pill */}
          <button
            onClick={() => { setFilter('category', 'all'); setSearchQuery(''); setSupabaseOnly(false); setAppsOnly(false); setPharmacyOnly(false); }}
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              filters.category === 'all' && !supabaseOnly && !appsOnly && !pharmacyOnly
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              {filters.category === 'all' ? 'Wellness Services' : getCurrentCategoryDisplay()}
              <svg
            ...
            {showCategoryDropdown && (
              <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto min-w-[180px]">
                {categories.filter(c => c !== 'All').map((category) => {
                  const categoryType = formatCategoryType(category)
                  const isSelected = filters.category === categoryType
                  return (
                    <button
                      key={category}
                      onClick={() => { setFilter('category', categoryType); setSearchQuery(''); setSupabaseOnly(false); setAppsOnly(false); setPharmacyOnly(false); setShowCategoryDropdown(false); }}
          ...
          {/* Pharmacy pill */}
          <button
            onClick={() => { setPharmacyOnly(true); setAppsOnly(false); setSupabaseOnly(false); setFilter('category', 'all'); setSearchQuery(''); }}
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              pharmacyOnly
                ? 'bg-emerald-500 text-white shadow-md'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
          ...
          {/* Saga Providers pill */}
          <button
            onClick={() => { setSupabaseOnly(true); setAppsOnly(false); setPharmacyOnly(false); setFilter('category', 'all'); setSearchQuery(''); }}
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              supabaseOnly
                ? 'bg-emerald-500 text-white shadow-md'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
          ...
          {/* Apps pill */}
          <button
            onClick={() => { setAppsOnly(true); setSupabaseOnly(false); setPharmacyOnly(false); setFilter('category', 'all'); setSearchQuery(''); }}
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              appsOnly
                ? 'bg-emerald-500 text-white shadow-md'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
        ...
        <div className="md:hidden flex items-center justify-center gap-2">
          ...
          {/* Apps pill */}
          <button
            onClick={() => { setAppsOnly(!appsOnly); if (!appsOnly) { setSupabaseOnly(false); setPharmacyOnly(false); } }}
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              appsOnly
                ? 'bg-emerald-500 text-white shadow-md'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
          ...
          {/* Pharmacy pill */}
          <button
            onClick={() => { setPharmacyOnly(!pharmacyOnly); if (!pharmacyOnly) { setAppsOnly(false); setSupabaseOnly(false); } }}
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              pharmacyOnly
                ? 'bg-emerald-500 text-white shadow-md'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
